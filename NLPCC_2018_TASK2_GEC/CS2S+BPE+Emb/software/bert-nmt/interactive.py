...
def make_batches(lines, args, task, max_positions, encode_fn):
    ...
    def getbert(line):
        line = line.strip()
        words = bertdict.encode(line, add_special_tokens=True)
        line = '{} {} {}'.format('[CLS]', line, '[SEP]')
        tokenizedline = bertdict.tokenize(line)
        # if len(tokenizedline) > bertdict.max_len:
        if len(tokenizedline) > bertdict.max_model_input_sizes:
            print('DCMMC: unexpected too long input')
            # tokenizedline = tokenizedline[:bertdict.max_len - 1]
            tokenizedline = tokenizedline[:bertdict.max_model_input_sizes - 1]
