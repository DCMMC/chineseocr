...
class Binarizer:
    ...
    def binarize(filename, dict, consumer, tokenize=tokenize_line, append_eos=True, reverse_order=False,
        ...
        def replaced_consumer_bert(word, idx):
            if idx == dict.vocab.get(dict.unk_token) and word != dict.unk_token:
                replaced.update([word])
        ...
        with open(filename, 'r', encoding='utf-8') as f:
            ...
            while line:
                ...
                if isinstance(dict, BertTokenizer):
                    line = line.strip()
                    words = dict.encode(line, add_special_tokens=True)
                    ...
                    for i, word in enumerate(words):
                        ids[i] = word
                        replaced_consumer_bert(tokenizedline[i], word)
                else:
#                     print('DCMMC: unexpected dict type in binarizer.py, must be BertTokenizer!')
