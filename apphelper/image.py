...
def solve(box):
    ...
    思路：按照与中心坐标关系，判断出左下和右下端点，假定 x1 到 x4 四个点是按照顺时针或逆时针顺序排列，
    判断出排列方向后按左上，左下，右下，右上排序排序，然后针对横排或竖排文字采用 x 轴平均或 y 轴平均尝试对齐
    """
    x1, y1, x2, y2, x3, y3, x4, y4 = box[:8]
    cx = (x1+x3+x2+x4)/4.0
    cy = (y1+y3+y4+y2)/4.0
    # DCMMC:用底边来判断 angle，不考虑文字方向，文字方向实在是不好考虑，在 ICPR MTWI 2018 数据集中
    # 主要是该数据集中很多不规则文本行例如盖章，只一个字作为一个 box的，而且每个 box 的文本方向都很凌乱，这里不做
    # 额外处理，以求 CRNN 有对各种方向的单字的识别能力
#     print('box:', box[:8])
...
#         print('横排文字')
        ...
        # 假定端点按照逆/顺时针顺序
        left_lower = max(
            list(filter(lambda p: p[0] < cx, location)), key=lambda x: x[1])
        right_lower = max(
            list(filter(lambda p: p[0] > cx, location)), key=lambda x: x[1])
        # 修正
        reverse = location.index(right_lower) - location.index(left_lower)
        x1, y1 = location[(location.index(left_lower) - reverse + 4) % 4]
        x2, y2 = left_lower
        x3, y3 = location[(location.index(left_lower) + reverse + 4) % 4]
        x4, y4 = location[(location.index(left_lower) + 2 * reverse + 4) % 4]
#         print(x1, y2, x2, y2, x3, y3, x4, y4)
        # 偏移线
        # 感觉底线不如平均线准确
#         location = [left_lower, location[(location.index(left_lower) + reverse + 4) % 4]]
        location = [[(x1 + x2) / 2, (y1 + y2) / 2], [(x3 + x4) / 2, (y3 + y4) / 2]]
        h_ = max(np.sqrt((x2-x1)**2+(y2-y1)**2), np.sqrt((x3-x4)**2+(y3-y4)**2))
        if h_ == 0:
            h_ = h
        w_ = max(np.sqrt((x2-x3)**2+(y2-y3)**2), np.sqrt((x1-x4)**2+(y1-y4)**2))
        if w_ == 0:
            w_ = w
    else:
        # 这时候我认为是竖排文字
        left_lower = min(
            list(filter(lambda p: p[1] > cy, location)), key=lambda x: x[0])
        right_lower = max(
            list(filter(lambda p: p[1] > cy, location)), key=lambda x: x[0])
...
#         print(x1, y2, x2, y2, x3, y3, x4, y4)
        # 偏移线
        # 感觉底线不如平均线准确, 这里用 y 平均线
#         location = [left_lower, location[(location.index(left_lower) + reverse + 4) % 4]]
        location = [[(y2 + y3) / 2, (x2 + x3) / 2], [(y1 + y4) / 2, (x1 + x4) / 2]]
        h_ = max(np.sqrt((x2-x1)**2+(y2-y1)**2), np.sqrt((x3-x4)**2+(y3-y4)**2))
        if h_ == 0:
            h_ = h
        w_ = max(np.sqrt((x2-x3)**2+(y2-y3)**2), np.sqrt((x1-x4)**2+(y1-y4)**2))
...
#         print('Unexpected location when solve the box')
        tanA = 0
    if abs(tanA) > 1.732:  # tan 60 degree
        # DCMMC: 期望的就是 x1, y1 对应左上角，在 ICPR MTWI 2018 数据集中
        print('Unexpected tanA!!!!')
    angle = np.arctan(tanA)
#     if abs(angle) < 5:
...
def rotate_cut_img(im, box, leftAdjustAlph=0.0, rightAdjustAlph=0.0):
    ...
    box = (max(1, cx-w/2-leftAdjustAlph*(w/2))  # xmin
           ...
           #            min(cx+w/2+rightAdjustAlph*(w/2),im.shape[0]-1)##xmax
           , cy+h/2)  # ymax
    newW = box[2]-box[0]
    newH = box[3]-box[1]
...
def adjust_box_to_origin(img, angle, result):
    ...
    for line in result:
        ...
        x1, y1, x2, y2, x3, y3, x4, y4 = box_rotate(
            [x1, y1, x2, y2, x3, y3, x4, y4], angle=(360-angle) % 360, imgH=imgH, imgW=imgW)
        box = x1, y1, x2, y2, x3, y3, x4, y4
        newresult.append(
